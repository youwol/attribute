import { Serie, createTyped, DataFrame, exists } from "@youwol/dataframe"
...
export class NormalsDecomposer implements Decomposer {
    ...
    serie(df: DataFrame, itemSize: number, name: string): Serie {
        ...
        indices.forEach( t => {
            ...
            const n = vec.cross(vec.create(v1, v2) as vec.Vector3, vec.create(v1, v3) as vec.Vector3)
            data[i++] = n[0]; data[i++] = n[1]; data[i++] = n[2]
        })
        
        return Serie.create({array: createTyped(Float32Array, data, true), itemSize: 3})//.setName(this.name)
